...
(function() {
  ...
  var days_of_week = [_('Su'), _('Mo'), _('Tu'), _('We'), _('Th'), _('Fr'),
                      _('Sa')];
  ...
  DateTimeField = Class.$extend({
    ...
    drawTimetable: function() {
      ...
      if (this.auto_show) {
        $('<a class="close">SchlieÃŸen</a>').click(function() {
          self.container.hide();
          return false;
        }).appendTo(col);
      }
        ...
        $('<tr></tr>').append(
          $('<th colspan="7" class="caption"></th>').append(
            ...
            $('<span>' + months[month-1] + ' ' + year + '</span>').click(function() {
              ...
              $(this).next().focus();
            }).attr('title', 'Klicke hier, um schnell einen anderen Monat auszuwÃ¤hlen')
          )
        )
      );
