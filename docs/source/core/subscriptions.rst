...
list” below). The list is not implemented yet.
...
:ref:`defining-subscriptions` below).
...
and subject, the action is the formation of that relation. Like subscription
types, actions are implemented by subclassing :class:`SubscriptionAction` (see
:ref:`defining-subscriptions` below).

.. _subscription-modes-verbose:
...
attributes that are needed to create the list of new objects and to know
whether a notification needs to be sent:
...
.. automethod:: Subscription.accessed(user, subject=None, object=None)

.. _defining-subscriptions:
