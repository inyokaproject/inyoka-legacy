...
class TwillRecordMiddleware(IMiddleware):
    ...
    def _generate_unittest_function(self):
        ...
        _buffer.append(u'    import twill; tc=twill.commands')
        _buffer.append(u'    from inyoka.core.test import _TwillBrowserProxy; b=_TwillBrowserProxy()')
        _buffer.append(u'')
    ...
    def process_response(self, request, response):
        ...
        if request.method.lower() == 'get':
            _buffer.append(u'    # check that we can access %s properly'
                             % request.current_url)
            _buffer.append(u'    tc.go("%s")' % request.current_url)
            _buffer.append(u'    tc.url("%s")' % request.current_url)
        elif request.method.lower() == 'post':
            counter = 1
            #TODO: try to check something here!
            _buffer.append(u'    # replay form submit')
            _buffer.append(u'    tc.formclear(%i)' % counter)
            fvl = u'    tc.formvalue("%s", "' % unicode(counter)
            for key, value in request.form.iteritems():
                _buffer.append(fvl + key + u'", """' + value + u'""")')
            _buffer.append(u'    tc.submit()')
